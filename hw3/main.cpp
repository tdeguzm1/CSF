...
int check_valid_inputs(int num_args, char* input_args[]);
...
int main(int argc, char* argv[]) {
  int error = check_valid_inputs(argc, argv);
  if (error) {
      return error;
  }
...
int check_valid_inputs(int num_args, char* input_args[]){
  ...
  else if (strcmp(input_args[5], "write-back") == 0 && strcmp(input_args[4], "no-write-allocate") == 0) {  // cannot no-write-allocate with write-back
